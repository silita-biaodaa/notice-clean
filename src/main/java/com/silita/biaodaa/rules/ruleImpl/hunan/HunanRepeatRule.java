...
//        delRelationInfoAndEditDetail(notice, historyNotice);
    ...
    public boolean executeRule(EsNotice esNotice) {
        ...
        try {
            ...
            //满足条件的，不进行去重,直接入库
            if(MyStringUtils.isNotNull(title) && title.length()<6){
                isNewNotice =true;//进公告表
            }else {
                //根据关键字截取标题，结合其他条件构造查询条件
                Map argMap = buildTitleMatchParam(esNotice);
                List<EsNotice> matchTitleList = noticeRuleService.matchEsNoticeList(argMap);
                if (matchTitleList.size() <= 0) {
                    isNewNotice = true;//无匹配重复公告，直接入库
                } else {
                    Set<EsNotice> matchSetList = new HashSet<EsNotice>(matchTitleList);//匹配总集合
                    //匹配公告集合：相同地区，相同类型，公示时间前后3天,相似度大于80%
                    argMap.remove("titleKey");
                    List<EsNotice> matchNoticeList = noticeRuleService.matchEsNoticeList(argMap);
                    for (EsNotice notice : matchNoticeList) {
                        if (similarDegreeWrapper(title, notice.getTitle()) > 0.8) {
                            matchSetList.add(notice);
                        }
                    }
                    ...
                    if(matchSetList.size()<=0){
                        ...
                        for (EsNotice notice : matchSetList) {
                        ...
                        if (sameTitleList.size() > 0) {
                            ...
                            while (iter.hasNext()) {
                                ...
                                if (extractUrlHost(url).equals(extractUrlHost(esnt.getUrl()))) {//网站相同
                                    ...
                                    if (esntPress != null && press != null
                                            && esntPress.equals(press)) {
                                        //保留新进来的公告,替换历史公告
                                        return replaceHistoryNotice(esnt, esNotice);
                                    } else {
                                        //公告入库
                                        isNewNotice = true;
                                    }
...
//                                                notSameTitleList.remove(esnt);
                                ...
                                if (notSameTitleList.size() > 0) {
                                    ...
                                    Iterator notSameIter2 = notSameTitleList.iterator();
            ...
            if(isNewNotice){
                //新公告无重复公告
                if(!repeatExecute.equals("replace")){
